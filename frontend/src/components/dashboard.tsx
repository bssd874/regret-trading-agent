...
function DecisionStream({
  ...
  const orderedDecisions = useMemo(
    () => [...decisions].sort((left, right) => Number(right.id === selectedId) - Number(left.id === selectedId)),
  ...
  return (
    <section className="min-h-[500px] border border-[#465154] bg-[#1b1f20] px-8 py-9 lg:px-11 lg:py-10" id="replay">
      ...
      <div className="grid gap-6 sm:grid-cols-[minmax(0,1fr)_230px] sm:items-start">
        <div>
          <p className="text-[19px] text-slate-100">{rejected ? copy.replay.saidNo : copy.replay.saidYes}</p>
          <p className="mt-3 min-h-7 text-[18px] leading-7 text-slate-300">
            {rejected ? copy.replay.rejectedQuestion(decision.symbol) : copy.replay.acceptedQuestion(decision.symbol)}
          </p>
        </div>
        <div className="sm:text-right">
          <p className={`editorial-number text-[27px] font-semibold tabular-nums ${classificationTone(event.classification)}`}>
            {formatCurrency(unsignedMagnitude(event.decision_value))}
          </p>
          <p className={`mt-1 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] sm:justify-end ${classificationTone(event.classification)}`}>
            <span className={`size-1.5 ${classificationMarker(event.classification)}`} />
      ...
      <div className="flex flex-col gap-6 pt-8 sm:flex-row sm:items-center sm:justify-between">
        <p className="max-w-[610px] text-[17px] leading-7 text-slate-200">{classificationNarrative(event, copy)}</p>
...
function AuditTrail({
  ...
  return (
    <section className="py-12" aria-label={copy.audit.title}>
      <h2 className="text-[18px] text-slate-100">{copy.audit.title}</h2>
      ...
      {loading ? (
        ...
        <>
          <ol className="relative mt-10 grid gap-y-7 sm:grid-cols-5">
            <div className="absolute left-5 right-5 top-[5px] hidden border-t border-[#333c3f] sm:block" />
            {[
              {
                label: copy.audit.marketScout,
                value: copy.audit.candidateDiscovered,
                detail: undefined,
              },
              {
                label: copy.audit.azureAnalyst,
                value: copy.audit.confidence(formatPercent(detail.analyst.confidence)),
                detail: undefined,
              },
              {
              ...
              {
                label: copy.audit.consensus,
                value: copy.audit.adjustedConfidence(formatPercent(detail.consensus.adjusted_confidence)),
                detail: undefined,
              },
              {
                label: copy.audit.riskGate,
                value: decision.decision === "REJECT" ? copy.audit.noTrade : decision.decision,
                detail: decision.decision === "REJECT" ? copy.audit.belowThreshold : detail.risk.reasons[0],
                risk: true,
              },
            ].map((item, index) => (
              <li className="relative px-3 text-center sm:px-5" key={item.label}>
...
export function Dashboard() {
  ...
  function replay() {
    ...
    setReplayStage(0);
    for (let nextStage = 1; nextStage <= 4; nextStage += 1) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1600px] px-5 pb-5 sm:px-7">
      ...
      <AuditTrail copy={copy} decision={selectedDecision} detail={detail} detailError={detailError} language={language} loading={detailLoading} stage={replayStage} />
